 ...
 * phase projects onto the InputBar — submit routing, visuals (token color /
...
describe('matrix row: plain', () => {
  ...
  it('primary submit falls to the default sink; no claim on the currency; edits free', async () => {
...
describe('matrix row: claimed', () => {
  ...
  it('primary submit routes to claim.submit (command lane, never the queue sink)', async () => {
    const submit = vi.fn(() => Promise.resolve({ kind: 'success' as const, text: '完成', source: 'command', name: 'goal' }))
    const { view, shell, sink, claim } = bench({ submit })
    claim()
...
describe('matrix row: claimed with attachments', () => {
  ...
  it('an accepting claim serializes and forwards a mixed batch; success consumes and clears', async () => {
    ...
    const { shell, claim, serialize, release } = bench({ submit, serialize: () => Promise.resolve([png, file]) })
    claim('/goal ', '目标', true)
  ...
  it('a handler error keeps a serialized file unreleased beside the notice and draft', async () => {
    ...
    const file: SubmitAttachment = { type: 'file', receiptId: 'receipt-x' }
    const { view, shell, claim, release } = bench({ submit, serialize: () => Promise.resolve([file]) })
    claim('/goal ', '目标', true)
    act(() => { shell.addAttachments([img]) })
    act(() => { shell.submit() })
  ...
  it('a serialize rejection blocks the transaction: notice, no submit call, images kept', async () => {
    const submit = vi.fn(() => Promise.resolve({ kind: 'success' as const }))
    const { view, shell, claim, release } = bench({ submit, serialize: () => Promise.reject(new Error('附件已失效')) })
    claim('/goal ', '目标', true)
    act(() => { shell.addAttachments([img]) })
    act(() => { shell.submit() })
  ...
  it('image removal is refused while a command submit is in flight', async () => {
    ...
    const { shell, claim } = bench({ submit, serialize: () => Promise.resolve([]) })
    claim('/goal ', '目标', true)
    act(() => { shell.addAttachments([img]) })
    act(() => { shell.submit() })
...
describe('matrix row: submitting', () => {
  it('locks submit, renders pending + read-only, keeps the claim snapshot on the currency', async () => {
    const submit = vi.fn(() => new Promise<SubmitOutcome>(() => {})) // never settles
    ...
    // Further submit (and Enter) stay dead inside the lock (submit dispatch is microtask-deferred).
