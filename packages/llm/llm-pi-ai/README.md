...
The OpenRouter catalog has a DSH fallback for [Space Bunny Alpha](https://openrouter.ai/stealth/space-bunny-alpha) under the id `stealth/space-bunny-alpha` while the installed pi-ai snapshot lacks it. The fallback includes OpenRouter's published context and output limits, text and image input, and reasoning efforts from `low` through `max`; once pi-ai ships the same id, its catalog entry takes precedence. The route still needs credentials with access to the model.
